...
static void activate(SharedMemory *shmem) {
    SharedMemory$write_uint(shmem, OFFSET_ACTIVE, 1);
}
static void deactivate(SharedMemory *shmem) {
    SharedMemory$write_uint(shmem, OFFSET_ACTIVE, 0);
}
static unsigned int active(SharedMemory *shmem) {
    return SharedMemory$read_uint(shmem, OFFSET_ACTIVE);
}

static void set_pid(SharedMemory *shmem, pid_t pid) {
    SharedMemory$write_uint(shmem, OFFSET_PID, pid);
}
static pid_t get_pid(SharedMemory *shmem) {
    return shmem ? SharedMemory$read_uint(shmem, OFFSET_PID) : 0;
}

static void set_cpid(SharedMemory *shmem, pid_t cpid) {
    SharedMemory$write_uint(shmem, OFFSET_CPID, cpid);
}
static pid_t get_cpid(SharedMemory *shmem) {
    return shmem ? SharedMemory$read_uint(shmem, OFFSET_CPID) : 0;
}

static pid_t get_count(SharedMemory *shmem) {
    return shmem ? SharedMemory$read_uint(shmem, OFFSET_COUNT) : 0;
}
static void inc_count(SharedMemory *shmem) {
    SharedMemory$write_uint(shmem, OFFSET_COUNT, get_count(shmem)+1);
}
